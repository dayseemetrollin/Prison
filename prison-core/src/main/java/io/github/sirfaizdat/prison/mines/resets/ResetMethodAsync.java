...
import io.github.sirfaizdat.prison.mines.AsyncResetCache;
 ...
 * @author SirFaizdat
 */
public class ResetMethodAsync implements ResetMethod {
        ...
        for (int x = minX; x <= maxX; x++)
            for (int y = minY; y <= maxY; y++)
                for (int z = minZ; z <= maxZ; z++) {
                    ...
                    for (MineComposition.CompositionEntry ce : mapComposition(mine)) {
    ...
    private List<MineComposition.CompositionEntry> mapComposition(Mine mine) {
        if(!AsyncResetCache.getInstance().has(mine))
            AsyncResetCache.getInstance().put(mine, mine.getComposition().mapComposition());
        return AsyncResetCache.getInstance().get(mine);
    }
